...
이름 명판(안쪽 64px)에서 긴 이름이 잘리던 문제는 전진폭을 7px로 줄이고,
그래도 넘치는 이름은 성 뒤 공백을 빼서 해결했습니다 (`이나와시로 모리쿠니` → `이나와시로모리쿠니`).
...
**코드 6바이트**로 문자 전진폭을 7px로 바꾸고, 한글 787자가 전부 잉크 폭 7px인
**갈무리11 Condensed**를 왼쪽 정렬로 주입해 해결했습니다.
창 넘침 517건 → **0건**입니다.

전진폭은 그리기 루프뿐 아니라 **문자열 폭 계산(`strwidth`)과 줄 수 계산**에도
따로 박혀 있습니다. 이 셋이 어긋나면 이름이 엉뚱한 x에서 시작하거나 창 밖으로
밀리므로 여섯 군데(`0x3EAC` `0x3EE4` `0x0AE6` `0x0AEC` `0x0B20` `0x0B2E`)를 전부 맞췄습니다.
