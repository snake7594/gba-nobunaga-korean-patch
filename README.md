...
| 패치본 MD5 | `618c2ac80e903751ebc63752cbed6b5f` |
...
`노부나가`가 들어가지 않는 문제는, 게임이 이미 갖고 있던 **반각 경로(1바이트·8px)** 에
...
**코드 1바이트**(`0x3EAC`: `0C`→`08`)로 전각 전진폭을 12→8px로 바꾸고,
한글 787자가 전부 잉크 폭 7px인 **갈무리11 Condensed**를 왼쪽 정렬로 주입해 해결했습니다.
창 넘침 517건 → 2건이 되었고, 남은 21건은 번역을 다듬어 **0건**입니다.
...
- 전각 전진폭을 8px로 줄여 글자가 원본보다 좁습니다. 가운데 정렬된 UI는 왼쪽으로 몰려 보일 수 있습니다.
